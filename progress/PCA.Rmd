...
데이터를 평균 0, 표준편차 1로 표준화한 후, 상관관계를 구하도록 한다.
...
R에서는 기본함수인 `prcomp()`, `princomp()` 함수를 통해 PCA 분석을 수행할 수 있으며, `psych` 패키지를 통해 더욱 다양한 분석을 할 수 있다.
...
5개 성분만으로도 충분한 설명력이 있는 것으로 보인다.

### 직각 회전과 해석

회전의 목적은 특정한 성분에 관해 변수의 기여도를 최대화함으로써 각 성분 사이의 상관 관계를 줄여 해석을 간단학 하는 것이다.
...
먼저 각 5개 성분에 관한 변수들의 기여도는 RC1부터 RC5까지 열로 구분되어 있다. RC1(성분1)은 **Goals_Against**와 **Shots_Against** 변수의 성분에 관한 기여도가 높은 양의 값이고, **PP_perc**와 **PK_perc** 변수의 기여도가 높은 음의 값이다.

RC2(성분2)은 **Goals_For** 변수가 높은 기여도를 가지고 있다. RC5는 **Shots_For, CF60_pp, OZFOperc_pp**가 높은 기여도를 갖고 있다. RC3는 **take** 변수만 연관이 있으며, RC4는 **hits** 변수와 연관이 있다.

SS loadings 제곱합으로 시작하는 표의 숫자는 각 성분의 고윳값(Eigenvalue)이다. 이 고윳값이 정규화되면 Proportion Explained 행의 값이며, 서분 1이 5개의 회전된 성분 모두가 설명하는 분산의 28%를 설명하는 것을 볼 수 있다.

경험적으로 선택된 성분들이 설명하는 분산의 총합이 최소한 전체 분산의 70%를 넘어야 하며, Cumulative Var 행의 5개 성분 합이 총 74%의 분산을 나타내고 있다.

### 요인 점수 생성

회전된 성분들의 기여도를 각 팀의 요인 점수로 변환한다. 
...
$R^2$가 70%에 달하며, p값이 1.446e-06로 나와 통계적으로 높은 유의성을 갖고 있다. 그러나 RC1과 RC2를 제외한 3개 성분은 유의하지 않은 것으로 보인다. 두 가지 피처만을 대상으로 회귀분석을 다시 실시한다.
...
$R^2$가 역시나 70%에 가까우며, 통계적으로 유의한 모델이다.
...
평균 제곱 오차의 제곱근을 계산한 후 테스트 셋과 비교해보도록 한다.
